...
<div class="portlet box light-grey">
    ...
    <div class="portlet-body">
        ...
        <div class="clearfix">
            ...
            <div style="display: inline-block;padding-left: 40px;float: right;" >
                <form action="<?php echo U('Notice/index');?>" method="post" style="margin: 0;">
                    <input type="text" name="search_title" value="<?php echo ($search["search_title"]); ?>" placeholder="请输入查询的公告标题"/>
        ...
        <table class="table table-striped table-bordered" id="sample_1">
            ...
            <?php if(!empty($list["list"])): if(is_array($list["list"])): $i = 0; $__LIST__ = $list["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="odd gradeX" id="<?php echo ($vo["id"]); ?>">
                    ...
                    <td data-name="title" data-id="<?php echo ($vo["id"]); ?>"><?php echo (msubstr($vo["title"],0,20)); ?></td>
                    <td   data-name="content"  data-id="<?php echo ($vo["id"]); ?>"> <?php echo (msubstr($vo["content"],0,40)); ?> </td>
                    ...
                    <td>
                        <?php if($vo["status"] == 1): ?><span class="label label-success">已发布 </span>
                            <?php else: ?><span class="label label-danger">未发布</span><?php endif; ?>
                    </td>
                    <td>
                        <a href="javascript:edit(<?php echo ($vo["id"]); ?>);">编辑</a>
                        <?php if($vo["status"] == 1): ?><a href="javascript:javascript:enable(<?php echo ($vo["id"]); ?>,0);">下线</a>
                            <?php else: ?> <a href="javascript:javascript:enable(<?php echo ($vo["id"]); ?>,1);">发布</a><?php endif; ?>
                    </td>
                    </td>
    ...
    <div id="addModal" style="min-width: 800px;" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true">
        ...
        <form action="<?php echo U('Member/create');?>" id="addForm" method="post" class="form">
            <div class="modal-body">
...
<!-- BEGIN CORE PLUGINS -->
    ...
    <script src="/Public/media/js/notice-managed.js"></script>

    <script>
        jQuery(document).ready(function() {
            viewUrl = "<?php echo U('Notice/view');?>";
            enableUrl = "<?php echo U('Notice/changeStatus');?>";
